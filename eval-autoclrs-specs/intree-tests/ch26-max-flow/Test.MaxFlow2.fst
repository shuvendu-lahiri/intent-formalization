...
let max_flow_caps_ok (cap_seq: Seq.seq int) : Lemma
  (requires
    ...
    Seq.index cap_seq 0 == 0 /\ Seq.index cap_seq 1 == 7 /\
...
=
  ...
  assert (Seq.index flow_seq 3 == 0)
    by (FStar.Tactics.norm [delta_only [`%get]];
        FStar.Tactics.smt ());
  assert (0 <= Seq.index flow_seq 1 /\ Seq.index flow_seq 1 <= 7)
    by (FStar.Tactics.norm [delta_only [`%get]];
        FStar.Tactics.smt ());
  if Seq.index flow_seq 1 = 7 then ()
  else begin
    assert (Seq.index flow_seq 1 < 7);
