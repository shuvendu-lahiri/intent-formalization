...
let completeness_bstarray_search (result: option SZ.t) (keys: Seq.seq int) (valid: Seq.seq bool) : Lemma
  (requires Seq.length keys == 1 /\
            ...
            (Some? result ==> (
              ...
              Seq.index keys (SZ.v (Some?.v result)) == 11)) /\
            (None? result ==> ~(key_in_subtree keys valid 1 0 11)))
  (ensures Some? result /\ SZ.v (Some?.v result) == 0)
= if None? result then begin
    assert (key_in_subtree keys valid 1 0 11)
  ...
  ensures emp
{
  ...
  let result = tree_search t #ks0 #vs0 #lo #hi 11 ctr;
  with ks1 vs1 cf.
    assert (A.pts_to t.keys ks1 ** A.pts_to t.valid vs1 ** GR.pts_to ctr cf **
            pure (Seq.length ks1 == 1 /\
                  ...
                  (Some? result ==> (
                    ...
                    Seq.index ks1 (SZ.v (Some?.v result)) == 11)) /\
                  (None? result ==> ~(key_in_subtree ks1 vs1 1 0 11))));
