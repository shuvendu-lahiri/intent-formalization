...
let bfs_chain_3 (sadj: Seq.seq int) : prop =
  Seq.length sadj == 9 /\
  Seq.index sadj 0 == 0 /\ Seq.index sadj 1 == 1 /\ Seq.index sadj 2 == 0 /\
  Seq.index sadj 3 == 0 /\ Seq.index sadj 4 == 0 /\ Seq.index sadj 5 == 1 /\
  Seq.index sadj 6 == 0 /\ Seq.index sadj 7 == 0 /\ Seq.index sadj 8 == 0

let no_edge_to_zero_v2 (sadj: Seq.seq int) (u: nat) : Lemma
  (requires bfs_chain_3 sadj /\ u < 3)
  (ensures ~(has_edge sadj 3 u 0))
=
  assert (~(has_edge sadj 3 u 0))

let edge_to_one_from_zero_v2 (sadj: Seq.seq int) (u: nat) : Lemma
  (requires bfs_chain_3 sadj /\ u < 3 /\ has_edge sadj 3 u 1)
  (ensures u == 0)
=
  assert (u == 0)

let edge_to_two_from_one_v2 (sadj: Seq.seq int) (u: nat) : Lemma
  (requires bfs_chain_3 sadj /\ u < 3 /\ has_edge sadj 3 u 2)
  (ensures u == 1)
=
  assert (u == 1)

let rec reach_zero_only_at_zero_v2 (sadj: Seq.seq int) (k: nat) : Lemma
  (requires bfs_chain_3 sadj /\ reachable_in sadj 3 0 0 k)
  (ensures k == 0)
  (decreases k)
=
  if k = 0 then ()
    ...
    FStar.Classical.exists_elim
      ...
      ()
      (fun (u: nat{u < 3 /\ reachable_in sadj 3 0 u (k - 1) /\ has_edge sadj 3 u 0}) ->
        no_edge_to_zero_v2 sadj u;
        assert false)

let reach_one_at_one_v2 (sadj: Seq.seq int) (k: nat) : Lemma
  (requires bfs_chain_3 sadj /\ reachable_in sadj 3 0 1 k)
  (ensures k == 1)
=
  if k = 0 then
    assert false
    ...
    FStar.Classical.exists_elim
      ...
      ()
      (fun (u: nat{u < 3 /\ reachable_in sadj 3 0 u (k - 1) /\ has_edge sadj 3 u 1}) ->
        edge_to_one_from_zero_v2 sadj u;
        reach_zero_only_at_zero_v2 sadj (k - 1);
        assert (k == 1))

let reach_two_at_two (sadj: Seq.seq int) (k: nat) : Lemma
  (requires bfs_chain_3 sadj /\ reachable_in sadj 3 0 2 k)
  (ensures k == 2)
=
  if k = 0 then
    assert false
    ...
    FStar.Classical.exists_elim
      ...
      ()
      (fun (u: nat{u < 3 /\ reachable_in sadj 3 0 u (k - 1) /\ has_edge sadj 3 u 2}) ->
        edge_to_two_from_one_v2 sadj u;
        reach_one_at_one_v2 sadj (k - 1);
        assert (k == 2))

let bfs_complete_v2 (sadj: Seq.seq int) (sdist: Seq.seq int) : Lemma
  (requires
    bfs_chain_3 sadj /\
    Seq.length sdist == 3 /\
    Seq.index sdist 0 == 0 /\
  ...
  (ensures
    ...
    Seq.index sdist 2 == 2)
=
  reach_one_at_one_v2 sadj (Seq.index sdist 1);
  reach_two_at_two sadj (Seq.index sdist 2)
  ...
  ensures emp
{
  ...
  assert (pure (reachable_in sadj 3 0 2 2));
