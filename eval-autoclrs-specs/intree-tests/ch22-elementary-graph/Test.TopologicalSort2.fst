...
(* ================================================================
   ...
   Graph: 3 vertices, edges: 0->1, 0->2 (ambiguous DAG)
   Adjacency matrix (flat, row-major): [0;1;1; 0;0;0; 0;0;0]
   Expected output attempt: [0; 1; 2]
   ================================================================ *)

(* --- Acyclicity lemma: prove the test graph has no cycle --- *)
#push-options "--fuel 8 --ifuel 4 --z3rlimit 400"
let no_cycle_3_v2 (adj: Seq.seq int) : Lemma
  (requires
    Seq.length adj == 9 /\
    Seq.index adj 0 == 0 /\ Seq.index adj 1 == 1 /\ Seq.index adj 2 == 1 /\
    Seq.index adj 3 == 0 /\ Seq.index adj 4 == 0 /\ Seq.index adj 5 == 0 /\
...
=
  ...
  assert (has_edge 3 adj 0 2 == true);
  assert (has_edge 3 adj 1 0 == false);
  assert (has_edge 3 adj 1 1 == false);
  assert (has_edge 3 adj 1 2 == false);
  assert (has_edge 3 adj 2 0 == false);
  assert (has_edge 3 adj 2 1 == false);
  assert (has_edge 3 adj 2 2 == false)
#pop-options

(* --- Completeness lemma: attempt to force the order [0;1;2] --- *)
#push-options "--fuel 8 --ifuel 4 --z3rlimit 400"
let completeness_topo_v2 (sout: Seq.seq int) (adj: Seq.seq int) : Lemma
  (requires
    Seq.length adj == 9 /\
    Seq.index adj 0 == 0 /\ Seq.index adj 1 == 1 /\ Seq.index adj 2 == 1 /\
    Seq.index adj 3 == 0 /\ Seq.index adj 4 == 0 /\ Seq.index adj 5 == 0 /\
...
fn test_topological_sort2 ()
  requires emp
  returns _: unit
  ensures emp
{
  // Input: 3x3 adjacency matrix for graph 0->1 and 0->2
  let v = V.alloc 0 9sz;
  V.to_array_pts_to v;
  ...
  // Set edges: 0->1 (index 1) and 0->2 (index 2)
  adj.(1sz) <- 1;
  adj.(2sz) <- 1;
  ...
  // Apply completeness lemma: attempt to force output to be [0; 1; 2]
  completeness_topo_v2 sout s0;
