...
= assert (forall (i j:nat). (i <= j) == Prims.op_LessThanOrEqual i j);
  ...
  assert_norm (Seq.length (Seq.seq_of_list [5; 1; 3]) == 3);
...
let input_is_513 (s0: Seq.seq int) : Lemma
  (requires
    Seq.length s0 == 3 /\
    Seq.index s0 0 == 5 /\
    Seq.index s0 1 == 1 /\
    Seq.index s0 2 == 3)
  (ensures Seq.equal s0 (Seq.seq_of_list [5; 1; 3]))
= Seq.lemma_eq_intro s0 (Seq.seq_of_list [5; 1; 3])

let completeness_sort3_v2 (s0 s: Seq.seq int) : Lemma
  (requires
    Seq.length s0 == 3 /\
    Seq.index s0 0 == 5 /\
    Seq.index s0 1 == 1 /\
