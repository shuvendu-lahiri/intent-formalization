...
let rod_cutting_test ()
  : Lemma (optimal_revenue (Seq.seq_of_list [2; 5; 9]) 3 == 9)
...
fn test_rod_cutting_basic ()
  ...
  ensures emp
{
  let pv = V.alloc (0 <: nat) 3sz;
  V.to_array_pts_to pv;
  let prices_arr = V.vec_to_array pv;
  rewrite (A.pts_to (V.vec_to_array pv) (Seq.create 3 (0 <: nat))) as (A.pts_to prices_arr (Seq.create 3 (0 <: nat)));
  A.pts_to_len prices_arr;
  assert (pure (A.length prices_arr == SZ.v 3sz));

  A.op_Array_Assignment prices_arr 0sz (2 <: nat);
  A.op_Array_Assignment prices_arr 1sz (5 <: nat);
  A.op_Array_Assignment prices_arr 2sz (9 <: nat);

  let nSz = 3sz;
  rod_3sz_pre ();

  let ctr = GR.alloc #nat 0;
  let result = rod_cutting prices_arr nSz ctr;
